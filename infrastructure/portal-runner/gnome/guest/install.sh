...
if [[ $EUID -ne 0 ]]; then
  echo "GNOME runner image installation requires root" >&2
  exit 1
fi
if [[ $# -ne 1 ]]; then
...
[Unit]
Description=Ephemeral RobotGo protected portal runner
After=gdm.service network-online.target robotgo-runner-egress.service
...
install -d -m 0755 /etc/dconf/db/robotgo.d
cat >/etc/dconf/db/robotgo.d/00-runner <<'EOF'
