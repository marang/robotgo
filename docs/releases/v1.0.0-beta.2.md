...
This is not a stable release. The repository's immutable upstream history
already contains `v1.0.0` through `v1.0.2`, so the independent fork's first
stable target is `v1.1.0`, preceded by `v1.1.0-rc.1`. API freeze,
permission-granted macOS scope, and the compatibility lifecycle remain open.
...
Use the explicit version shown above for reproducibility. The repository
retains historical upstream stable `v1.x` tags for attribution and history,
but those commits declare `module github.com/go-vgo/robotgo` and are not valid
versions of the independent module. As verified on 2026-07-27, `go list -m
github.com/marang/robotgo@latest` resolves `v1.0.0-beta.2`. The retained tags
must not be moved or reused; see the
[stable release plan](../plan/stable-release-readiness.md).
