...
| Hyprland/Wayland | Native CGO, `wayland` | supported active-window subset | Blocking isolated `Hyprland E2E` evidence uses one verified GitHub-hosted `vkms` card and a self-owned fixture to prove title, PID, exact active bounds, legacy parity, explicit unsupported portable-handle/client/PID-target contracts, sanitizer execution, and cleanup; broader foreign-window state/control remains capability-gated |
