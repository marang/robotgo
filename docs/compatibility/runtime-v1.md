...
| Hyprland/Wayland | Native CGO, `wayland` | supported active-window subset | Retained exact-commit [`Hyprland E2E` run 30256070306](https://github.com/marang/robotgo/actions/runs/30256070306) uses one verified GitHub-hosted `vkms` card and a self-owned fixture to prove title, PID, exact active bounds, legacy parity, explicit unsupported portable-handle/client/PID-target contracts, sanitizer execution, and induced-failure cleanup; broader foreign-window state/control remains capability-gated |
