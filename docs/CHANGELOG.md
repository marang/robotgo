...
- Split KDE hosted-session readiness into bounded prerequisite, Plasma Shell,
  ...
  unit restart; recovery and ready claims are serialized with in-lock contract
  revalidation without unbounded phase resets, and a second failure remains
  terminal. KDE receives its own bounded timeout hierarchy while GNOME
