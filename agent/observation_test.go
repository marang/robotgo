...
func TestWaylandCatalogDoesNotAdvertiseImplicitPortalCapture(t *testing.T) {
	...
	waitCapability := buildCatalog(policy, capabilities).Operations[6]
	...
	findCapability := buildCatalog(policy, capabilities).Operations[5]
	...
	if waitCapability = buildCatalog(policy, capabilities).Operations[6]; !waitCapability.Available {
		t.Fatalf("active ScreenCast wait capability = %+v", waitCapability)
	}
	if findCapability = buildCatalog(policy, capabilities).Operations[5]; !findCapability.Available {
