...
func TestWaylandCatalogDoesNotAdvertiseImplicitPortalCapture(t *testing.T) {
	...
	waitCapability := buildCatalog(policy, capabilities).Operations[7]
	...
	findCapability := buildCatalog(policy, capabilities).Operations[6]
	...
	if waitCapability = buildCatalog(policy, capabilities).Operations[7]; !waitCapability.Available {
		t.Fatalf("active ScreenCast wait capability = %+v", waitCapability)
	}
	if findCapability = buildCatalog(policy, capabilities).Operations[6]; !findCapability.Available {
