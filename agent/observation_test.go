...
func TestWaylandCatalogDoesNotAdvertiseImplicitPortalCapture(t *testing.T) {
	...
	waitCapability := buildCatalog(policy, capabilities).Operations[8]
	...
	findCapability := buildCatalog(policy, capabilities).Operations[7]
	...
	if waitCapability = buildCatalog(policy, capabilities).Operations[8]; !waitCapability.Available {
		t.Fatalf("active ScreenCast wait capability = %+v", waitCapability)
	}
	if findCapability = buildCatalog(policy, capabilities).Operations[7]; !findCapability.Available {
